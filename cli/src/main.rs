...
const HELP_SECTIONS: &str = "Events:\n  \
                             ...
                             XDG_CONFIG_HOME, LIGHTINGSEND_ALIAS, LIGHTINGSEND_PORT, LIGHTINGSEND_DESTINATION";
...
mod tests {
    ...
    fn accepts_mixed_send_paths() {
        let args = Args::try_parse_from(["lightingsend-cli", "send", "one.txt", "two.txt", "backup"])
            .unwrap();
    ...
    fn accepts_a_destination_alias() {
        let args =
            Args::try_parse_from(["lightingsend-cli", "send", "--to", "Cute Tomato", "one.txt"])
                .unwrap();
    ...
    fn accepts_a_destination_ip() {
        let args =
            Args::try_parse_from(["lightingsend-cli", "send", "--to", "192.168.27.26", "one.txt"])
                .unwrap();
    ...
    fn requires_at_least_one_send_path() {
        assert!(Args::try_parse_from(["lightingsend-cli", "send"]).is_err());
    }

    #[test]
    fn accepts_interactive_mode_without_a_command() {
        let args = Args::try_parse_from(["lightingsend-cli"]).unwrap();
