...
ARM_NAME="LightingSend-CLI-$VERSION-macos-arm-64"
X64_NAME="LightingSend-CLI-$VERSION-macos-x86-64"
...
echo
cargo build --release --package lightingsend-cli --target aarch64-apple-darwin
cargo build --release --package lightingsend-cli --target x86_64-apple-darwin
...
cp target/aarch64-apple-darwin/release/lightingsend-cli "$ARM_NAME/lightingsend-cli"
cp target/x86_64-apple-darwin/release/lightingsend-cli "$X64_NAME/lightingsend-cli"
...
codesign --force --verbose --options runtime --sign "$SIGN_ID" "$ARM_NAME/lightingsend-cli"
codesign --force --verbose --options runtime --sign "$SIGN_ID" "$X64_NAME/lightingsend-cli"
...
echo
zip -j "$ARM_NAME.zip" "$ARM_NAME/lightingsend-cli"
zip -j "$X64_NAME.zip" "$X64_NAME/lightingsend-cli"
...
echo
tar -czvf "$ARM_NAME.tar.gz" -C "$ARM_NAME" lightingsend-cli
tar -czvf "$X64_NAME.tar.gz" -C "$X64_NAME" lightingsend-cli
