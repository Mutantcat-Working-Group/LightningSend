...
codesign --deep --force --verbose --options runtime --preserve-metadata=entitlements --sign "$SIGN_ID" build/macos/Build/Products/Release/LightingSend.app
...
rm -f "$DMG"
create-dmg \
  --volname "LightingSend" \
  --window-size 500 300 \
  --background "../support/build/dmg/background.png" \
  --icon LightingSend.app 130 110 \
  --app-drop-link 360 110 \
  "$DMG" \
  build/macos/Build/Products/Release/LightingSend.app
